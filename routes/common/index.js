...
utils.post('/sms', async (req, res) => {
	...
	const month = currentTime.getMonth() < 10 ? '0' + (currentTime.getMonth() + 1).toString():currentTime.getMonth() + 1
	const day = currentTime.getDay() < 10 ? '0' + (currentTime.getDay() + 1).toString() : currentTime.getDay().toString() + 1 
	const hours = currentTime.getHours() < 10 ? '0' + currentTime.getHours().toString() : currentTime.getHours().toString()
	const minutes = currentTime.getMinutes() < 10 ? '0' + currentTime.getMinutes().toString() : currentTime.getMinutes().toString()
	const seconds = currentTime.getSeconds() < 10 ? '0' + currentTime.getSeconds().toString() : currentTime.getSeconds().toString()
	const timestamp = year+month+day+hours+minutes+seconds

	const smsSet = await setSchema.findOne({type: 'sms'})
	const signStr = smsSet.sets.get('appKey') + timestamp + smsSet.sets.get('secretKey')
